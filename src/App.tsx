...
export default function App() {
  ...
  async function salvarEdicao(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('enxoval').update({
      ...
      local_compra: editando.local_compra,
      data_compra: editando.data_compra, 
      condicao: editando.condicao, 
      status: editando.status, 
      categoria: editando.categoria, 
      quem_presenteou: editando.quem_presenteou // Confirmado conforme banco de dados
    }).eq('id', editando.id);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 w-full flex flex-col overflow-x-hidden">
      ...
      {/* HEADER FIXO COM CONTRASTE */}
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-slate-200 px-4 py-3 w-full shadow-sm">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2">
             ...
             <div className="bg-green-50 px-2 py-1 rounded-lg border border-green-100">
               <span className="text-[10px] font-black text-green-700">R$ {totalGasto.toFixed(2)}</span>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-2">
          <div className="col-span-8 relative">
            ...
            <input className="w-full rounded-xl bg-slate-100 py-2 pl-9 pr-4 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 text-slate-900 font-bold" placeholder="Buscar..." value={busca} onChange={e => setBusca(e.target.value)} />
          </div>
          <div className="col-span-4 relative">
            <select 
              value={categoriaAtiva} 
              onChange={(e) => setCategoriaAtiva(e.target.value)}
              className="w-full bg-indigo-600 text-white text-[10px] font-bold py-2 px-2 rounded-xl appearance-none outline-none border-none shadow-sm"
            >
      ...
      <main className="p-3 space-y-3 w-full flex-1 mb-24">
        {loading && !editando ? ( <div className="text-center py-10 text-slate-400 text-xs font-bold animate-pulse uppercase">Sincronizando...</div> ) : (
          itensFiltrados.map((item) => (
            <div key={item.id} onClick={() => setEditando(item)} className="bg-white rounded-2xl p-4 shadow-md border border-slate-100 flex flex-col gap-2 active:bg-slate-50 active:scale-[0.98] transition-all">
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex flex-col gap-0.5">
                    ...
                    {item.status === 'Presente' && <span className="text-[11px] font-bold text-pink-600 uppercase italic">💝 {item.quem_presenteou || 'Alguém especial'}</span>}
                  </div>
                </div>
              </div>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 px-10 py-4 flex justify-around items-center z-30 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
      ...
      {/* MODAL EDIÇÃO OTIMIZADO (RESOLVE O VÍDEO) */}
      {editando && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-end sm:items-center justify-center overflow-hidden">
          <form onSubmit={salvarEdicao} className="bg-white w-full max-w-md rounded-t-[2.5rem] sm:rounded-3xl flex flex-col max-h-[95vh] shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-40">
              <div className="flex justify-center">
                <div className="relative h-32 w-32 rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden shadow-inner group transition-all">
