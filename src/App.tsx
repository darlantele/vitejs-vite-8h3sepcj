  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 w-full flex flex-col overflow-hidden">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 py-3 w-full shadow-sm shrink-0">
        ...
        <div className="grid grid-cols-12 gap-2">
          <div className="col-span-8 relative">
            ...
            <input className="w-full rounded-xl bg-slate-100 py-3 pl-9 pr-4 text-base outline-none focus:ring-2 focus:ring-indigo-500/20 text-slate-900 font-bold placeholder:text-slate-400" placeholder="Buscar..." value={busca} onChange={e => setBusca(e.target.value)} />
          </div>
          <div className="col-span-4 relative">
            <select value={categoriaAtiva} onChange={(e) => setCategoriaAtiva(e.target.value)} className="w-full bg-indigo-600 text-white text-[10px] font-bold py-3.5 px-2 rounded-xl appearance-none outline-none shadow-sm h-full">
              {categorias.map(cat => <option key={cat} value={cat}>{cat.toUpperCase()}</option>)}
            </select>
            <ChevronDown className="absolute right-2 top-4 h-3 w-3 text-white pointer-events-none" />
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-3 space-y-3 w-full pb-32">
        {loading && !editando ? ( <div className="text-center py-10 text-slate-400 text-xs font-bold animate-pulse uppercase tracking-widest">Sincronizando...</div> ) : (
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 px-10 py-4 pb-8 flex justify-around items-center z-30 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] shrink-0">
      ...
      {/* MODAL DE EDIÇÃO FIXADO AO TOPO (RESOLVE BABADDORES ESCONDIDO) */}
      {editando && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-start justify-center overflow-hidden">
          <form onSubmit={salvarEdicao} className="bg-white w-full max-w-md flex flex-col h-full shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            {/* CABEÇALHO DO MODAL - SEMPRE VISÍVEL NO TOPO */}
            <div className="flex justify-between items-center p-6 pt-10 border-b border-slate-100 shrink-0">
              <h2 className="text-sm font-black text-indigo-600 uppercase tracking-widest truncate pr-4">{editando.item_nome}</h2>
              <button type="button" onClick={() => setEditando(null)} className="p-3 bg-slate-100 rounded-full text-slate-500 active:bg-slate-200"><X size={24}/></button>
            </div>
            
            {/* CORPO ROLÁVEL - CAMPOS NÃO FLUTUAM MAIS */}
            <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-40">
              <div className="flex justify-center">
                <div className="relative h-36 w-36 rounded-[2.5rem] bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden shadow-inner transition-transform active:scale-95">
              ...
              <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl shadow-inner">
                {['Pendente', 'Comprado', 'Presente'].map(s => (
                  <button key={s} type="button" onClick={() => setEditando({...editando, status: s as any})} className={`flex-1 py-4 rounded-xl text-[10px] font-black transition-all ${editando.status === s ? 'bg-white shadow-sm text-indigo-700' : 'text-slate-500'}`}>
                    {s.toUpperCase()}
                  </button>
                ))}
              </div>
                ...
                <div className="space-y-8 animate-in fade-in duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2 text-left text-base">
                      ...
                      <input className="w-full bg-slate-100 p-4 rounded-2xl text-base font-bold outline-none border-2 border-transparent focus:border-indigo-400 shadow-sm" value={editando.marca || ''} onChange={e => setEditando({...editando, marca: e.target.value})} />
                    </div>
                    <div className="space-y-2 text-left">
                      <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Preço (R$)</label>
                      <input type="number" step="0.01" className="w-full bg-slate-100 p-4 rounded-2xl text-base font-black text-indigo-700 outline-none shadow-sm" value={editando.preco_pago || ''} onChange={e => setEditando({...editando, preco_pago: e.target.value})} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-left">
                    <div className="space-y-2">
                      ...
                      <input className="w-full bg-slate-100 p-4 rounded-2xl text-base font-bold outline-none shadow-sm" value={editando.local_compra || ''} onChange={e => setEditando({...editando, local_compra: e.target.value})} />
                    </div>
                    <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl h-16 mt-auto shadow-inner">
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-4 text-left">
                  <div className="space-y-2">
                    ...
                    <select className="w-full bg-slate-100 p-4 rounded-2xl text-base font-black appearance-none outline-none shadow-sm" value={editando.categoria || 'Outros'} onChange={e => setEditando({...editando, categoria: e.target.value})}>
                ...
                {editando.status === 'Presente' && (
                  <div className="space-y-2 text-left animate-in zoom-in-95 duration-200">
                    <label className="text-[10px] font-black text-pink-600 uppercase ml-1 tracking-widest">Quem presenteou o bebê?</label>
      ...
      {mostrarModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-start justify-center p-0 overflow-hidden">
          <div className="bg-white w-full max-w-md h-full flex flex-col animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center p-6 pt-10 border-b border-slate-100 shrink-0">
              <h2 className="text-base font-black text-slate-900 uppercase">Novo Item</h2>
              <button onClick={() => setMostrarModal(false)} className="p-3 bg-slate-100 rounded-full text-slate-500 active:bg-slate-200"><X size={24}/></button>
            </div>
            <div className="flex-1 p-8 space-y-8">
              <div className="flex gap-3">
                <input autoFocus className="flex-1 bg-slate-100 p-5 rounded-2xl outline-none text-base font-bold border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner" placeholder="O que vamos comprar?" value={novoItemNome} onChange={e => setNovoItemNome(e.target.value)} />
                <button onClick={() => escutarVoz(setNovoItemNome)} className="bg-indigo-50 text-indigo-600 p-5 rounded-2xl active:bg-indigo-100"><Mic size={28}/></button>
              </div>
